...
| [`incident-response-scheduled-workflow-failure.md`](./incident-response-scheduled-workflow-failure.md) | Manually detect and respond to a silent job failure (or missing run) in `build-index.yml`, `validate-schema.yml`, `cncf-mission-gen.yml`, `cncf-install-gen.yml`, `scan-missions.yml`, `platform-install-gen.yml`, `fuzz.yml`, `codeql.yml`, `scorecard.yml`, or `stale.yml` — none of which currently alert on the job itself failing. |
| [`fuzz-yml-ci-summary-gap.md`](./fuzz-yml-ci-summary-gap.md) | Preserved diff and rationale for adding a structured `$GITHUB_STEP_SUMMARY` to `fuzz.yml`'s three test steps — blocked on `workflows` token permission; tracked in #3316. |
