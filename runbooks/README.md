...
| [`incident-response-scheduled-workflow-failure.md`](./incident-response-scheduled-workflow-failure.md) | Manually detect and respond to a silent job failure (or missing run) in `build-index.yml`, `validate-schema.yml`, `cncf-mission-gen.yml`, `cncf-install-gen.yml`, or `scan-missions.yml` — none of which currently alert on the job itself failing. |
