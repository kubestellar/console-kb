...
| [`incident-response-unsafe-mission-merge.md`](./incident-response-unsafe-mission-merge.md) | Detect and recover from a mission merged to `master` via the `CNCF Mission Generation` workflow's `--admin` auto-merge, which bypasses branch protection without checking `Mission Safety Scan` or `Validate Mission Schema` results — and separately, from `Mission Safety Scan`'s own false-green on `runbooks/**`-only PRs, from `Validate Mission Schema` never validating `runbooks/**` at all (neither on PRs nor on its own scheduled sweep), from `KB Quality Enforcement` never scoring `runbooks/**`-only PRs, and from `Mission Content Validation` never validating `runbooks/**`-only PRs despite triggering on them. |
| [`incident-response-scheduled-workflow-failure.md`](./incident-response-scheduled-workflow-failure.md) | Manually detect and respond to a silent job failure (or missing run) in `build-index.yml`, `validate-schema.yml`, `cncf-mission-gen.yml`, `cncf-install-gen.yml`, `scan-missions.yml`, `platform-install-gen.yml`, `fuzz.yml`, `codeql.yml`, `scorecard.yml`, or `stale.yml` — none of which currently alert on the job itself failing — plus the same failure mode in event-triggered reusable-workflow callers, with `pr-verifier.yml`'s confirmed, currently-active 100% failure rate (#3336) as a worked example. |
