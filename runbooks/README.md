...
| [`incident-response-unsafe-mission-merge.md`](./incident-response-unsafe-mission-merge.md) | Detect and recover from a mission merged to `master` via the `CNCF Mission Generation` workflow's `--admin` auto-merge, which bypasses branch protection without checking `Mission Safety Scan` or `Validate Mission Schema` results. |
