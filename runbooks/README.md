...
| [`incident-response-unsafe-mission-merge.md`](./incident-response-unsafe-mission-merge.md) | Detect and recover from a mission merged to `master` via the `CNCF Mission Generation` workflow's `--admin` auto-merge bypassing `Mission Safety Scan`/`Validate Mission Schema` (fixed in #3418), from `Validate Mission Schema` never validating `runbooks/**` (fixed in #3410), and from `Mission Safety Scan` skipping `runbooks/**` in its own scan logic (fixed in #3444) — retained for historical incident recovery — plus the still-open false-green gaps in `KB Quality Enforcement` and `Mission Content Validation` on `runbooks/**`-only PRs. |
...
| [`fuzz-yml-ci-summary-gap.md`](./fuzz-yml-ci-summary-gap.md) | **Resolved (historical context only)** — preserved diff and rationale for the `$GITHUB_STEP_SUMMARY`/step-`id`/`if: always()` gap in `fuzz.yml`'s three test steps (#3316); PR #3431 fixed the gap with a different, already-merged implementation, so this diff is not a pending action. |
