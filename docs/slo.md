...
- **SLO**: 100% — a check with this name and a green checkmark should mean the suite
  passed, not "informational only." **Formerly known gap, now fixed**: the `Run
  tests` step in `scripts-tests.yml` used to carry `continue-on-error: true`,
  added intentionally while 4 tests were failing on `master` (#3023) with an
  explicit follow-up to remove it once the suite went green — tracked as
  [#3199](https://github.com/kubestellar/console-kb/issues/3199) and fixed in PR
  [#3417](https://github.com/kubestellar/console-kb/pull/3417) (merged
  2026-09-16), which dropped the flag and switched `npm test` to
  `vitest run --coverage` with a coverage summary appended to
  `$GITHUB_STEP_SUMMARY`. Verified on current `master`: the `Run tests` step in
  `.github/workflows/scripts-tests.yml` has no `continue-on-error`, so a real
  test failure now fails the check instead of merging behind a green
  checkmark.
...
Separately, `fuzz.yml` (daily, `0 6 * * *`, plus every PR/push to `master`)'s
structured-CI-observability gap tracked as
[#3316](https://github.com/kubestellar/console-kb/issues/3316) is now fixed:
PR [#3431](https://github.com/kubestellar/console-kb/pull/3431) (merged
2026-09-17) replaced the inline `node --input-type=module` heredocs with
calls to the standalone `scripts/fuzz-json-fixtures.mjs` and
`scripts/fuzz-mission-scanner.mjs` scripts, added step `id`s, and added an
`if: always()` "Post fuzz summary" step that writes a per-step results table
to `$GITHUB_STEP_SUMMARY` via `scripts/render-ci-step-summary.mjs`. Verified
on current `master`: `.github/workflows/fuzz.yml`'s final step is exactly
this summary. The diff preserved in
[`runbooks/fuzz-yml-ci-summary-gap.md`](../runbooks/fuzz-yml-ci-summary-gap.md)
was never applied as such — #3431 solved the same gap with a different,
already-merged implementation — so that runbook is now historical context
only, not a pending action.
...
Separately, the section 5 gap above (`scripts-tests.yml`'s `Run tests` step
carrying `continue-on-error: true`) is now resolved: PR
[#3417](https://github.com/kubestellar/console-kb/pull/3417) removed the
flag and added coverage measurement, closing
